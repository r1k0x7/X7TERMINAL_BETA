...
import { Newspaper, ExternalLink, Clock, Tag, Info } from 'lucide-react';
...
export default function NewsPanel() {
  ...
  const isMockData = newsItems.length > 0 && newsItems[0].url === 'https://example.com/1';
    ...
    <div className="space-y-4">
      ...
      {isMockData && (
        <div className="terminal-panel p-3 flex items-center gap-2 text-xs text-terminal-warning">
          ...
          <span>Using demo news. Add NewsAPI or World News API key for real news.</span>
        </div>
      )}
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {categories.map((cat) => (
          <button 
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap ${activeCategory === cat ? 'bg-terminal-accent text-black' : 'bg-terminal-panel border border-terminal-border text-terminal-muted'}`}
          >
            {cat}
          </button>
        ))}
