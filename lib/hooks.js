      ...
      try {
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            if (data.c && data.s) updateCryptoPrice(data.s, parseFloat(data.c), parseFloat(data.P));
          } catch (err) {}
        };
        ws.onclose = () => {
          if (isActive) { setWsConnected(false); reconnectTimeout = setTimeout(connect, 5000); }
        };
        ws.onerror = () => { if (isActive) { setWsConnected(false); if (ws) ws.close(); } };
      } catch (err) { if (isActive) setWsConnected(false); }
    };
    connect();
    return () => {
      isActive = false;
...
export function useTradeStream(symbol) {
  ...
  useEffect(() => {
    ...
    return () => { isActive = false; if (ws) { try { ws.close(); } catch (e) {} } };
  }, [symbol, addTrade]);
}
...
export function useFundingRate(symbol) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try { const data = await fetchFundingRate(symbol); if (data) setFunding(data); } catch (err) {}
    };
    fetchData();
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const assets = data.map((coin) => ({
          ...
          image: coin.image || '', // ← TAMBAH INI: Icon dari CoinGecko!
        }));
    ...
    const fetchData = async () => {
      try {
        ...
        const rates = pairs.map((pair) => {
          ...
          return { pair: `USD${pair}`, rate, change: 0, bid: rate * 0.9995, ask: rate * 1.0005 };
        });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!isActive) return;
        if (!articles || articles.length === 0) { setNewsItems([]); return; }
        const news = articles.map((article, index) => ({
          id: article.id || `news-${index}`, title: article.title || 'No Title',
          source: article.source?.name || article.source || 'Unknown',
          publishedAt: article.publishedAt || new Date().toISOString(),
          category: article.category || 'Finance', sentiment: article.sentiment || 'neutral', url: article.url || '#',
        }));
        if (isActive) setNewsItems(news);
      } catch (err) { if (isActive) setNewsItems([]); }
    };
    fetchData();
    const interval = setInterval(fetchData, 300000);
    return () => { isActive = false; clearInterval(interval); };
        ...
        const results = await Promise.all(
          FRED_SERIES.map(async (series) => {
            try {
              ...
              return { ...series, value: currentValue, date: latest.date || 'N/A', change };
            } catch (err) {
              const mock = getMockMacro().find(m => m.id === series.id);
              return mock || { ...series, value: 0, date: 'N/A', change: 0 };
            }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isActive) {
          setEarthquakes(eqData.map((eq) => ({
            mag: eq.properties?.mag || 0, place: eq.properties?.place || 'Unknown',
            time: eq.properties?.time || Date.now(),
            lat: eq.geometry?.coordinates?.[1] || 0, lon: eq.geometry?.coordinates?.[0] || 0,
          })));
        }
        const weatherData = await Promise.all(CITIES.map(async (city) => {
          try {
            ...
            if (!data) return null;
            return { city, temp: data.main?.temp || 20, condition: data.weather?.[0]?.main || 'Unknown', humidity: data.main?.humidity || 50, wind: data.wind?.speed || 0 };
          } catch (err) { return null; }
        }));
        if (isActive) setWeather(weatherData.filter(Boolean));
      } catch (err) {
        if (isActive) { setEarthquakes([]); setWeather(getMockWeather()); }
      } finally {
        if (isActive) setLoading(false);
      }
    };
