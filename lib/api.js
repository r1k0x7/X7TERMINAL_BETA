...
// API Keys - untuk static export, ENV harus di-bake saat build
...
export async function fetchStockQuote(symbol) {
  console.log('FINNHUB_KEY available:', !!FINNHUB_KEY); // Debug

  if (!FINNHUB_KEY) {
    console.warn('No Finnhub API key, using mock data for', symbol);
    const mock = MOCK_STOCKS.find(s => s.symbol === symbol);
    return mock || { c: 0, d: 0, dp: 0, v: 0 };
  }

  try {
    const url = `${FINNHUB_API}/quote?symbol=${symbol}&token=${FINNHUB_KEY}`;
    console.log('Fetching:', url); // Debug

    const response = await fetch(url, { next: { revalidate: 30 } });
    if (!response.ok) {
      console.warn(`Finnhub error ${response.status} for ${symbol}`);
      const mock = MOCK_STOCKS.find(s => s.symbol === symbol);
      return mock || { c: 0, d: 0, dp: 0, v: 0 };
    }
    const data = await response.json();
    console.log('Finnhub response:', data); // Debug
    return data;
  } catch (error) {
    console.warn(`fetchStockQuote(${symbol}) error:`, error);
    const mock = MOCK_STOCKS.find(s => s.symbol === symbol);
    return mock || { c: 0, d: 0, dp: 0, v: 0 };
  }
}
...
export async function fetchForexRates(base = 'USD') {
  try {
    ...
    console.warn('fetchForexRates error:', error);
    return null;
  }
}
...
export async function fetchNewsAPI(query = 'finance', pageSize = 20) {
  // Try World News API first (works in production)
  if (WORLDNEWS_KEY) {
    try {
      const res = await fetch(
        `${WORLDNEWS_API}/search-news?text=${encodeURIComponent(query)}&number=${pageSize}&api-key=${WORLDNEWS_KEY}`,
        { next: { revalidate: 300 } }
      );
      if (res.ok) {
      ...
      console.warn('World News API error:', err);
    }
  }

  // Try NewsAPI (localhost only, but try anyway)
  if (NEWS_KEY) {
    try {
  ...
  // Fallback: mock data
  console.warn('No news API key, using mock data');
  return MOCK_NEWS;
}
...
export async function fetchFREDData(seriesId) {
  if (!FRED_KEY) {
    console.warn('No FRED API key, using mock data for', seriesId);
    const mock = MOCK_MACRO.find(m => m.id === seriesId);
    return mock ? {
      observations: [
        { date: mock.date, value: String(mock.value) },
        { date: mock.date, value: String(mock.value * 0.99) },
      ]
    } : { observations: [] };
  }
  ...
  try {
    ...
    console.warn(`fetchFREDData(${seriesId}) error:`, error);
    const mock = MOCK_MACRO.find(m => m.id === seriesId);
    return mock ? {
      observations: [
        { date: mock.date, value: String(mock.value) },
...
export async function fetchEarthquakes(timeRange = 'day') {
  try {
    ...
    console.warn('fetchEarthquakes error:', error);
    return [];
  }
}
...
export async function fetchWeather(city) {
  if (!OWM_KEY) {
    console.warn('No OpenWeatherMap API key, using mock data for', city);
    const mock = MOCK_WEATHER.find(w => w.city === city);
    return mock ? {
      main: { temp: mock.temp, humidity: mock.humidity },
      weather: [{ main: mock.condition }],
      wind: { speed: mock.wind },
    } : null;
  }
  ...
  try {
    ...
    console.warn(`fetchWeather(${city}) error:`, error);
    const mock = MOCK_WEATHER.find(w => w.city === city);
    return mock ? {
      main: { temp: mock.temp, humidity: mock.humidity },
      weather: [{ main: mock.condition }],
...
export async function fetchFundingRate(symbol) {
  try {
    ...
    console.warn('fetchFundingRate error:', error);
    return null;
  }
}
